...
try:
    ...
    print '\n', '####     Invalid use     ####', '\n'
    print 'Usage = Multi_One.py -i <multi:1> -o <output prefix>'
    print 'For help use Multi_One.py -h'
    sys.exit(99)
...
for opt, arg in opts:
    if opt == '-h':
        print '\n', 'Select only one orthologous from a multi:1 orthologous data (reported by OMA pairwise analyses).', '\n'
        print 'Usage = Multi_One.py -i <multi:1> -o <output prefix>'
        print 'Where: -i = table as the output from pairwise orthology analyses in OMA but only with multi:1 orthologous type'
